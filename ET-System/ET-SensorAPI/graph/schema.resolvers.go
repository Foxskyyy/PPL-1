...
func (r *mutationResolver) Register(ctx context.Context, displayName string, username string, email string, password string) (*string, error) {
...
func (r *mutationResolver) AssignUserToGroup(ctx context.Context, email string, userGroupID int32) (*string, error) {
...
func (r *mutationResolver) OauthLogin(ctx context.Context, provider model.OAuthProvider, token string) (*model.AuthPayload, error) {
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		username := providerID
		if email != "" {
			username = strings.Split(email, "@")[0] + "_" + providerStr
		}

		// var count int64
		baseUsername := username
		for i := 1; ; i++ {
			err = config.DB.Where("username = ?", username).First(&models.User{}).Error
			if errors.Is(err, gorm.ErrRecordNotFound) {
				break
			}
			if err != nil {
